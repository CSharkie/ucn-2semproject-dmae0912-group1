...
public class EmployeeGUI extends Composite {
	...
	public EmployeeGUI(Composite parent, int style) {
		super(parent, style);
		...
		btn_save.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				String type = null;
				try {
					employeeId = Integer.parseInt(txt_id.getText());
					...
					if (!error) {
						boolean ok = true;
						try {
							employeeId = persCtr.insertEmployee(firstName,
				...
				if (!create) {
					...
					if (!error) {
						boolean ok = true;
						try {
							persCtr.updateEmployee(employeeId, firstName,
									surName, address, phoneNo, email, salary,
									type);
						} catch (Exception ex1) {
							MessageBox box = new MessageBox(getShell(), 0);
		...
		btn_delete.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (!error) {
					try {
						persCtr.removeEmployee(employeeId);
					} catch (Exception ex) {
						MessageBox box = new MessageBox(getShell(), 0);
						box.setText("Error");
						box.setMessage("There was an error. Please try again");
		...
		txt_type.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				1, 1));
	...
	private void showAllEmployees() {
		...
		LinkedList<Employee> employees = persCtr.getAllEmployees();
		for (Employee employee : employees) {
			TableItem item = new TableItem(table, SWT.NONE);
			item.setText(0, String.valueOf(employee.getPersonId()));
			item.setText(1, employee.getFirstName());
			item.setText(2, employee.getSurName());
		}

	}

	private void showSearchedEmployees(String name) {
		table.clearAll();
		table.setItemCount(0);
		LinkedList<Employee> employees = persCtr.searchEmployeeByName(name);
		for (Employee employee : employees) {
			TableItem item = new TableItem(table, SWT.NONE);
			item.setText(0, String.valueOf(employee.getPersonId()));
			item.setText(1, employee.getFirstName());
	...
	private void showEmployee(int id) {
		Employee employee = persCtr.searchEmployeeById(id);
		txt_id.setText(String.valueOf(employee.getPersonId()));
		txt_firstName.setText(employee.getFirstName());
		txt_surName.setText(employee.getSurName());
		txt_address.setText(employee.getAddress());
		txt_email.setText(employee.getEmail());
		txt_phoneNo.setText(employee.getPhoneNo());
		txt_salary.setText(String.valueOf(employee.getSalary()));
		txt_type.setText(employee.getType());
