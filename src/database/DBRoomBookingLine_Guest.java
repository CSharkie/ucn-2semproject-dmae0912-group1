...
public class DBRoomBookingLine_Guest implements IFDBRoomBookingLine_Guest {
	...
	// insert a new Room
	public int insertGuest(int RoomBookingLineId, int GuestId) throws Exception { // call
																					// to
																					// get
		// the next Id
		// number
		...
		String query = "INSERT INTO RoomBookingLine_Guest(RoomBookingLine_GuestId, RoomBookingLineId, GuestId)  VALUES('"
				+ nextId + "','" + RoomBookingLineId + "','" + GuestId + "');";
	...
	public int deleteGuest(int RoomBookingLineId, int GuestId) {
		...
		String query = "DELETE FROM RoomBookingLine_Guest WHERE RoomBookingLineId = '"
				+ RoomBookingLineId + "' AND GuestId = '" + GuestId + "'";
		...
		try { // delete from RoomBookingLine_Guest
			...
			System.out.println("Delete exception in RoomBookingLine_Guest db: "
					+ ex);
		}
		return (rc);
	}
	...
	// private methods
	// michWere is used whenever we want to select more than one
	// RoomBookingLine_Guest
	...
	private LinkedList<Guest> miscWhere(String wClause,
		...
		try { // read the Room from the database
			...
			while (results.next()) {
				...
				Guest guest = dbGuest.searchGuestById(
						results.getInt("GuestId"), false);
				list.add(guest);
			}// end while
			stmt.close();
